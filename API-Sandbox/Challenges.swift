...
internal func exerciseOne() {
    ...
    print("\(firstName) \(lastName) lives at \(streetName) in \(city), \(state), \(postCode)." +
     " If you want to contact \(title). \(lastName), you can email \(email) or call at  \(cellPhoneNumber).")
}
...
internal func exerciseThree() {
    ...
    for disneyMovie in allMovies {
        if disneyMovie.rightsOwner.containsString("Disney"){
    ...
    for newMovie in allMovies{
        if newMovie.releaseDate.containsString("2016"){
